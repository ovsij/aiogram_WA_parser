...
def inline_kb_menu(telegram_user):
    ...
    text = markdown.text(
        ...
        '',
        'Вы можете ознакомиться с нашим каталогом, а если вы не нашли то, что вам нужно, присылайте фото желаемой вещи нам - @ConciergeShopping, а наши байеры выкупят ее для Вас по самой лучшей цене',
        sep='\n')
...
def inline_kb_categories(tg_id : str, metacategory : int, page : int = 1):
    ...
    text = 'КАТАЛОГ'
    if get_metacategory(id=metacategory).name == 'Все бренды':
...
def inline_kb_listproducts(tg_id : str, category : int = None, sub_category : int = None, sizes : str = None, prices : str = None, page : list = [0, 5], back : bool = False, sort : str = None, search : str = None):
    ...
    for product in products[page[0]:page[1]]:
        ...
        category_ = get_category(id=category)
        dct['text'] = f'{category_.name}\n\n{product.name}\n\n{description}\n\nЦена: {price} руб.'
...
def inline_kb_terms(tg_id):
    text = markdown.text(
        ...
        '',
        'Сроки доставки рассчитываются индивидуально: зависят от страны отправки и города доставки (для РФ средний срок доставки 2-3 недели)',
        sep='\n'
    )
    text_and_data = [
        btn_back('menu')
...
def inline_kb_howto():
    text = markdown.text(
        markdown.bold('Вы нашли ту самую вещь, но не знаете, как её заказать?'),
        'Предлагаем 2 варианта: вы можете сделать заказ через бота @Concierge_Shopping_bot или написать нашим менеджерам напрямую в Telegram @ConciergeShopping\.',
        markdown.bold('Какой способ выбрать?'),
        '1\. *Заказ через бота* удобен тем, что максимально приближен к классическому формату онлайн\-шопинга\. Следуя простой инструкции вы выбираете понравившийся товар, добавляете его в корзину, оставляете ваши контактные данные и вносите 100% предоплату за товар\.',
        'После оформления заказа с вами свяжутся наши специалисты для уточнения деталей заказа\.',
        '2\. *Заказ напрямую через менеджера* подойдет в случае, если вы не нашли желаемый товар в нашем Telegram\-канале или боте\. Для оформления заказа необходимую предоставить всю информацию о товаре: фото, цвет, размер, бренд, и т\. д\. Наши менеджеры помогут вам на каждом этапе совершения покупки и ответят на все ваши вопросы\.',
        markdown.bold('Важно!'),
        '• Мы принимаем оплату картами платежных систем Visa, Mastercard, МИР и переводом Usdt Trc\-20\.',
        '• Если желаемый товар сразу в наличии — вы переводите 100% предоплату и наши менеджеры связываются с вами для уточнения деталей заказа\. А для случаев, когда прямо сейчас понравившийся вам товар не в наличии, у нас предусмотрен депозит\.',
        markdown.italic('Депозит - фиксированная сумма, которая закрепляется за вами и повышает скорость выкупа необходимого вам товара, когда он будет в наличии.'),
        '• Сроки и стоимость доставки рассчитываются индивидуально и зависят от страны отправки и города доставки\. В среднем 40\-60€ за посылку, в ней может быть как товар для одного человека, так и для нескольких, что позволяет разбить стоимость доставки между людьми\. '
        'При оформлении заказа через бот доставка в любом случае платная\.',
        'Доставка по России осуществляется компанией СДЭК, в ПВЗ или до двери\. Также предусмотрена доставка Почтой России \(при оформлении индивидуального заказа\)\.',
        sep='\n\n'
        )
    text_and_data = [btn_back('menu')]
    schema = [1]
